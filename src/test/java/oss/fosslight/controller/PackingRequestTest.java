...
public class PackingRequestTest {
    ...
    @Test
    void testOssNotice() throws Exception {
        OssNotice ossNotice = new OssNotice();

        ossNotice.setPrjId(prj.getPrjId());
        ossNotice.setNoticeType("10");
        ossNotice.setAppended("");
        ossNotice.setAppendedTEXT("");
        ossNotice.setNetworkServerFlag("N");
        ossNotice.setPackageJson("");
        ossNotice.setPackageFileId("");
        ossNotice.setEditCompanyYn("Y");

        String content = new ObjectMapper().writeValueAsString(ossNotice);

        mockMvc.perform(post("/project/verification/saveAjax")
                .content(EntityUtils.toString(new UrlEncodedFormEntity(Arrays.asList(
                        new BasicNameValuePair("OssNotice",content)
                ))))
                .param("useCustomNoticeYn","N")
                .param("allowDownloadNoticeHTMLYn", "Y")
                .param("allowDownloadNoticeTextYn", "N")
