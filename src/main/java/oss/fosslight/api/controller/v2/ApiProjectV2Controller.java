...
//			paramMap.put("userRole", userInfo.getAuthority());
	...
	@GetMapping(value = {APIV2.FOSSLIGHT_API_PROJECT_CREATE})
		...
		try {
			...
			if (searchFlag) {
				...
				if (binaryTxt != null) {
						...
						binartTxtBean = apiFileService.uploadFile(binaryTxt); // file 등록 처리 이후 upload된 file정보를 return함.
	...
	@GetMapping(value = {APIV2.FOSSLIGHT_API_PROJECT_ADD_WATCHER})
