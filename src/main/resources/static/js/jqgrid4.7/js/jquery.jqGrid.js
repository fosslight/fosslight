...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			_index : {},
			grouping : false,
			groupingView : {groupField:[],groupOrder:[], groupText:[],groupColumnShow:[],groupSummary:[], showSummaryOnHide: false, sortitems:[], sortnames:[], summary:[],summaryval:[], plusicon: 'ui-icon-plus', minusicon: 'ui-icon-minus', displayField: [], groupSummaryPos:[], formatDisplayField : [], _locgr : false},
			ignoreCase : false,
			cmTemplate : {},
			idPrefix : "",
			multiSort :  false,
			...
			if(pgboxes) {
				...
				if (ts.p.viewrecords){
					if(ts.p.reccount === 0) {
						...
						if(ts.p.defaultRowNum != null) {
							var ossbase, ossto, ossfrom;
							ossbase = parseInt(ts.p.page,10)-1;
							ossbase = ossbase*parseInt(ts.p.defaultRowNum,10);
							ossfrom = ossbase+1;
							ossto = ossbase + parseInt(ts.p.defaultRowNum,10);
							if(ossto > ts.p.records) ossto = ts.p.records;
							ossfrom = $.fmatter.util.NumberFormat(ossfrom,fmt);
							ossto = $.fmatter.util.NumberFormat(ossto,fmt);
							$(".ui-paging-info",pgboxes).html($.jgrid.format(ts.p.recordtext,ossfrom,ossto,tot));
						} else {
							$(".ui-paging-info",pgboxes).html($.jgrid.format(ts.p.recordtext,from,to,tot));
						}
					}
...
"use strict";	
	...
	$.fn.fmatter.email = function(cellval, opts) {
		if(!$.fmatter.isEmpty(cellval)) {
			return "<a "+target+" class='urlLink' href=\"" + cellval + "\">" + cellval + "</a>";
		}
		return $.fn.fmatter.defaultFormat(cellval,opts);
	};
	$.fn.fmatter.link2 = function(cellval, opts) {
		...
		if(!$.fmatter.isEmpty(cellval)) {
			return "<a title=\"" + cellval + "\" class='urlLink' target=\"_blank\" href=\"" + cellval + "\">"+"URL</a>";
		}
		return $.fn.fmatter.defaultFormat(cellval,opts);
	};
	$.fn.fmatter.linkOssName = function(cellval, opts) {
		var op = {target:opts.target};
		var target = "";
		if(opts.colModel !== undefined && opts.colModel.formatoptions !== undefined) {
			op = $.extend({},op,opts.colModel.formatoptions);
	...
	$.fn.fmatter.linkLicenseName = function(cellval, opts) {
		var op = {target:opts.target};
		var target = "";
		if(opts.colModel !== undefined && opts.colModel.formatoptions !== undefined) {
			op = $.extend({},op,opts.colModel.formatoptions);
	...
	$.fn.fmatter.linkVulnPopOssName = function(cellval, opts, rowObject) {
		var op = {target:opts.target};
		var target = "";
		if(opts.colModel !== undefined && opts.colModel.formatoptions !== undefined) {
			op = $.extend({},op,opts.colModel.formatoptions);
		...
		if(!$.fmatter.isEmpty(cellval)) {
			...
			return "<a "+target+" class='urlLink' href=\"javascript:;\" onclick=\"opener.createTabInFrameWithCondition('"+_frameId+"','"+_frameTarget+"','VULNOSSNAME','"+_valParam+"')\" >" + cellval + "</a>";
		}
		return $.fn.fmatter.defaultFormat(cellval,opts);
	};
	$.fn.fmatter.ossType = function(cellval, opts) {
		var op = {target:opts.target};
		var target = "";
		if(opts.colModel !== undefined && opts.colModel.formatoptions !== undefined) {
			op = $.extend({},op,opts.colModel.formatoptions);
		...
		if(!$.fmatter.isEmpty(cellval)) {
			var colHtml=''; 
			if(cellval.indexOf('M') != -1){
				colHtml+='<span class="iconSet multi">Multi</span>';
			}
			if(cellval.indexOf('D') != -1){
				colHtml+='<span class="iconSet dual"> Dual</span>';
			}
			if(cellval.indexOf('V') != -1){
				colHtml+='<span class="iconSet vdif"> v-Diff</span>';
			}
			return colHtml;
		}
		return $.fn.fmatter.defaultFormat(cellval,opts);
	};
	$.fn.fmatter.vuln = function(cellval, opts, rowObject) {
		var op = {target:opts.target};
		var target = "";
		if(opts.colModel !== undefined && opts.colModel.formatoptions !== undefined) {
			op = $.extend({},op,opts.colModel.formatoptions);
		...
		if(op.target) {target = 'target=' + op.target;}
		if(!$.fmatter.isEmpty(cellval)) {
			if(parseInt(cellval) >= 9.0){
				return "<span class=\"iconSet vulCritical\" onclick=\"openNVD('"+ rowObject.cveId +"')\">"+cellval+"</span>";
			}else if(parseInt(cellval) >= 7.0){
				return "<span class=\"iconSet vulHigh\" onclick=\"openNVD('"+ rowObject.cveId +"')\">"+cellval+"</span>";
			}else if(parseInt(cellval) >= 4.0){
				return "<span class=\"iconSet vulMiddle\" onclick=\"openNVD('"+ rowObject.cveId +"')\">"+cellval+"</span>";
			}else if(parseInt(cellval) > 0){
				return "<span class=\"iconSet vulLow\" onclick=\"openNVD('"+ rowObject.cveId +"')\">"+cellval+"</span>";
			}else{
				return "<span style=\"font-size:0;\">"+cellval+"</span>";
			}
		}
		return $.fn.fmatter.defaultFormat(cellval,opts );
	};
