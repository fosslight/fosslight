...
-- Fresh installs from v2.0.0–v2.1.0 create.sql may have FULLTEXT KEY DOWNLOAD_LOCATION.
-- Upgrade path via 20240725150921 only creates INDEX OSS_COMMON_ID (no DOWNLOAD_LOCATION).
ALTER TABLE `OSS_DOWNLOADLOCATION` DROP INDEX IF EXISTS `DOWNLOAD_LOCATION`;
...
DELETE FROM `T2_CODE_DTL` WHERE CD_NO = '102' AND CD_DTL_NO = '817';
...
DELETE FROM `T2_CODE_DTL` WHERE CD_NO = '110' AND CD_DTL_NO = '71';
