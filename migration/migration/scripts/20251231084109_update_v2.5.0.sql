...
INSERT INTO `T2_CODE` (`CD_NO`, `CD_NM`, `CD_EXP`, `SYS_CD_YN`) VALUES ('211', '3rd Party Status', '', 'N');
INSERT INTO `T2_CODE_DTL` (`CD_NO`, `CD_DTL_NO`, `CD_DTL_NM`, `CD_SUB_NO`, `CD_DTL_EXP`, `CD_ORDER`, `USE_YN`) VALUES
...
UPDATE `T2_CODE_DTL` SET CD_DTL_NM = 'SELECT T0.OSS_ID, T1.OSS_NAME, T1.OSS_VERSION, T2.CVE_ID, T2.CVSS_SCORE, T2.VULN_SUMMARY, DATE_FORMAT(T2.MODI_DATE, \'%Y-%m-%d\') AS MODI_DATE, DATE_FORMAT(T2.PUBL_DATE, \'%Y-%m-%d\') AS PUBL_DATE FROM OSS_DISCOVERED_SND_EMAIL T0 INNER JOIN OSS_MASTER T1 ON T0.OSS_ID = T1.OSS_ID AND T0.SND_YN = \'N\' AND T1.USE_YN = \'Y\' LEFT JOIN NVD_CVE_V3 T2 ON T0.CVE_ID = T2.CVE_ID WHERE ? GROUP BY T0.OSS_ID, T2.CVE_ID ORDER BY T1.OSS_NAME, T1.OSS_VERSION, T2.CVSS_SCORE DESC' WHERE CD_NO = '104' AND CD_DTL_NO = '208';
